...
def analyse(chat_path: str):
    ...
    while exists == True:
        ...
        if Path(my_file).is_file():
            ...
            person_messages_desc = sorted(person_messages.items(), key=lambda item: item[1], reverse=True)
            print(person_messages_desc)
            print("\n\n\n")

            words_used_desc =  sorted(words_used.items(), key=lambda item: item[1], reverse=True) 
            print(words_used_desc[:20])
            break
